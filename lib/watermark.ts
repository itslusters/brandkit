...
// Watermark spec: "BRANDKIT" text, opacity 0.18, rotated -30°, scaled to 70% of input width.
...
export async function applyWatermark(imageBuffer: Buffer): Promise<Buffer> {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${wmWidth}" height="${wmHeight}" viewBox="0 0 ${wmWidth} ${wmHeight}">
    <g transform="rotate(-30 ${wmWidth / 2} ${wmHeight / 2})">
      <text x="50%" y="50%" font-family="Helvetica, Arial, sans-serif" font-size="${Math.round(wmHeight * 0.7)}" font-weight="900" text-anchor="middle" dominant-baseline="middle" fill="white" fill-opacity="0.18">BRANDKIT</text>
    </g>
  </svg>`
