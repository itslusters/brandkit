...
// Heavy native modules — mock them so vitest doesn't need binaries
vi.mock('sharp', () => {
  const chain = {
    resize: () => chain,
    png: () => chain,
...
vi.mock('jszip', () => {
  class MockJSZip {
    file = vi.fn()
    generateAsync = vi.fn().mockResolvedValue(Buffer.from('mock-zip'))
  }
...
vi.mock('@/lib/vector', () => ({
  pngToSvg: vi.fn().mockResolvedValue('<svg/>'),
}))

vi.mock('@/lib/recraft', () => ({
  vectorizeRecraftImage: vi.fn().mockResolvedValue('<svg/>'),
}))

import { auth } from '@clerk/nextjs/server'
import { POST } from '@/app/api/brand/assets/generate/route'
...
describe('POST /api/brand/assets/generate', () => {
  ...
  it('returns a ZIP response on success', async () => {
    const res = await POST(makeRequest())
